...
class AVLTree {
  ...
  private insertNode(node: Optional<AVLTreeNode>, key: HashRingNode) {
    ...
    if (balance > 1 && node.leftChild && key.position < node.leftChild.key.position) {
    ...
    if (balance < -1 && node.rightChild && key.position > node.rightChild.key.position) {
  ...
  private handleNodeDeletion(node: AVLTreeNode) {
    ...
    node.rightChild = this.deleteNode(node.rightChild, inOrderSuccessor.key);
    return node;
  }
  ...
  private deleteNode(root: Optional<AVLTreeNode>, key: HashRingNode) {
    ...
    // Left Left Case
    if (balance > 1 && root.leftChild && root.leftChild.subTreeHeightDifference >= 0) {
    ...
    // Right Right Case
    if (balance < -1 && root.rightChild && root.rightChild.subTreeHeightDifference <= 0) {
