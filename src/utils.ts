...
function visualizeHashRing(params: VisualizeHashRingParams) {
  ...
  if (ringNodes.length > 0) {
    ...
    const sampleKeys = ["user:1001", "product:5432", "session:abc123", "order:9876", "comment:45678", "post:1234"];

    for (const key of sampleKeys) {
      const hash = generateHash(key);
      const node = getPhysicalNode(key);
      output += `  ${key} → ${node?.nodeId || "No node"} (0x${hash.toString(16).padStart(8, "0")})\n`;
    }
  }
